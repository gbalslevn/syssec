...
public class OpenSSLSignature extends SignatureSpi {
    ...
    static abstract class RSAPKCS1Padding extends OpenSSLSignature {
        RSAPKCS1Padding(long j5) {
            super(j5, EngineType.RSA, null);
        }

        @Override // org.conscrypt.OpenSSLSignature
        protected final void configureEVP_PKEY_CTX(long j5) {
            NativeCrypto.EVP_PKEY_CTX_set_rsa_padding(j5, 1);
        }
    }
    ...
    static abstract class RSAPSSPadding extends OpenSSLSignature {
        ...
        RSAPSSPadding(long j5, String str, int i5) {
            super(j5, EngineType.RSA, null);
            this.contentDigestAlgorithm = str;
            this.mgf1DigestAlgorithm = str;
            this.mgf1EvpMdRef = j5;
            this.saltSizeBytes = i5;
        }

        @Override // org.conscrypt.OpenSSLSignature
        protected final void configureEVP_PKEY_CTX(long j5) {
            NativeCrypto.EVP_PKEY_CTX_set_rsa_padding(j5, 6);
            NativeCrypto.EVP_PKEY_CTX_set_rsa_mgf1_md(j5, this.mgf1EvpMdRef);
            NativeCrypto.EVP_PKEY_CTX_set_rsa_pss_saltlen(j5, this.saltSizeBytes);
        }
        ...
        protected final AlgorithmParameters engineGetParameters() {
            try {
                ...
                throw new ProviderException("Failed to create PSS AlgorithmParameters", e5);
            } catch (InvalidParameterSpecException e6) {
                throw new ProviderException("Failed to create PSS AlgorithmParameters", e6);
            }
        }
        ...
        protected final void engineSetParameter(AlgorithmParameterSpec algorithmParameterSpec) {
            ...
            try {
                ...
                throw new ProviderException("Failed to obtain EVP_MD for " + jcaDigestAlgorithmStandardName2, e5);
            }
        }
    }
    ...
    public static final class SHA256RSAPSS extends RSAPSSPadding {
        public SHA256RSAPSS() {
            super(EvpMdRef.SHA256.EVP_MD, Constants.SHA256, EvpMdRef.SHA256.SIZE_BYTES);
        }
    }
    ...
    /* synthetic */ OpenSSLSignature(long j5, EngineType engineType, AnonymousClass1 anonymousClass1) {
        this(j5, engineType);
    }

    private void checkEngineType(OpenSSLKey openSSLKey) {
        int EVP_PKEY_type = NativeCrypto.EVP_PKEY_type(openSSLKey.getNativeRef());
        int i5 = AnonymousClass1.$SwitchMap$org$conscrypt$OpenSSLSignature$EngineType[this.engineType.ordinal()];
        if (i5 == 1) {
            if (EVP_PKEY_type == 6) {
                return;
            }
    ...
    private void initInternal(OpenSSLKey openSSLKey, boolean z5) {
    ...
    protected void configureEVP_PKEY_CTX(long j5) {
    }
    ...
    private OpenSSLSignature(long j5, EngineType engineType) {
    ...
    protected void engineUpdate(byte[] bArr, int i5, int i6) {
        ...
        if (this.signing) {
            NativeCrypto.EVP_DigestSignUpdate(evp_md_ctx, bArr, i5, i6);
        } else {
            NativeCrypto.EVP_DigestVerifyUpdate(evp_md_ctx, bArr, i5, i6);
        }
    }
        ...
        if (byteBuffer.hasRemaining()) {
            ...
            if (position >= 0) {
                ...
                if (remaining >= 0) {
                    NativeRef.EVP_MD_CTX evp_md_ctx = this.ctx;
                    if (this.signing) {
                        NativeCrypto.EVP_DigestSignUpdateDirect(evp_md_ctx, j5, remaining);
                    } else {
                        NativeCrypto.EVP_DigestVerifyUpdateDirect(evp_md_ctx, j5, remaining);
                    }
