...
public abstract class BaseLayer implements DrawingContent, BaseKeyframeAnimation.AnimationListener, KeyPathElement {
    ...
    private void applyMasks(Canvas canvas, Matrix matrix) {
        ...
        for (int i5 = 0; i5 < this.mask.getMasks().size(); i5++) {
            ...
            BaseKeyframeAnimation<ShapeData, Path> baseKeyframeAnimation = this.mask.getMaskAnimations().get(i5);
            BaseKeyframeAnimation<Integer, Integer> baseKeyframeAnimation2 = this.mask.getOpacityAnimations().get(i5);
            ...
            if (i6 != 1) {
                if (i6 == 2) {
                    if (i5 == 0) {
                        ...
                        this.contentPaint.setAlpha(Function.USE_VARARGS);
                        canvas.drawRect(this.rect, this.contentPaint);
                    }
                    if (mask.isInverted()) {
    ...
    private boolean areAllMasksNone() {
        ...
        for (int i5 = 0; i5 < this.mask.getMasks().size(); i5++) {
            if (this.mask.getMasks().get(i5).getMaskMode() != Mask.MaskMode.MASK_MODE_NONE) {
                return false;
            }
        }
        return true;
    ...
    private void intersectBoundsWithMask(RectF rectF, Matrix matrix) {
        ...
        if (hasMasksOnThisLayer()) {
            ...
            for (int i5 = 0; i5 < size; i5++) {
                Mask mask = this.mask.getMasks().get(i5);
                Path value = this.mask.getMaskAnimations().get(i5).getValue();
                if (value != null) {
                    this.path.set(value);
                    this.path.transform(matrix);
                    int i6 = AnonymousClass1.$SwitchMap$com$airbnb$lottie$model$content$Mask$MaskMode[mask.getMaskMode().ordinal()];
    ...
    private void recordRenderTime(float f5) {
        this.lottieDrawable.getComposition().getPerformanceTracker().recordRenderTime(this.layerModel.getName(), f5);
    }
    ...
    private void setupInOutAnimations() {
        ...
        this.inOutAnimation.addUpdateListener(new BaseKeyframeAnimation.AnimationListener() { // from class: c0.a
    ...
    public void draw(Canvas canvas, Matrix matrix, int i5) {
        ...
        int intValue = (int) ((((i5 / 255.0f) * ((opacity == null || (value = opacity.getValue()) == null) ? 100 : value.intValue())) / 100.0f) * 255.0f);
    ...
    abstract void drawLayer(Canvas canvas, Matrix matrix, int i5);

    public BlurEffect getBlurEffect() {
        return this.layerModel.getBlurEffect();
    }

    public BlurMaskFilter getBlurMaskFilter(float f5) {
        if (this.blurMaskFilterRadius == f5) {
            return this.blurMaskFilter;
        }
        BlurMaskFilter blurMaskFilter = new BlurMaskFilter(f5 / 2.0f, BlurMaskFilter.Blur.NORMAL);
        this.blurMaskFilter = blurMaskFilter;
        this.blurMaskFilterRadius = f5;
    ...
    void resolveChildKeyPath(KeyPath keyPath, int i5, List<KeyPath> list, KeyPath keyPath2) {
    }
    ...
    public void resolveKeyPath(KeyPath keyPath, int i5, List<KeyPath> list, KeyPath keyPath2) {
        ...
        if (baseLayer != null) {
            ...
            if (keyPath.fullyResolvesTo(this.matteLayer.getName(), i5)) {
                list.add(addKey.resolve(this.matteLayer));
            }
            if (keyPath.propagateToChildren(getName(), i5)) {
                this.matteLayer.resolveChildKeyPath(keyPath, keyPath.incrementDepthBy(this.matteLayer.getName(), i5) + i5, list, addKey);
            }
        }
        if (keyPath.matches(getName(), i5)) {
            ...
            if (keyPath.propagateToChildren(getName(), i5)) {
                resolveChildKeyPath(keyPath, i5 + keyPath.incrementDepthBy(getName(), i5), list, keyPath2);
            }
        }
    }
    ...
    public void setOutlineMasksAndMattes(boolean z5) {
        if (z5 && this.outlineMasksAndMattesPaint == null) {
    ...
    public void setProgress(float f5) {
        this.transform.setProgress(f5);
        if (this.mask != null) {
            for (int i5 = 0; i5 < this.mask.getMaskAnimations().size(); i5++) {
                this.mask.getMaskAnimations().get(i5).setProgress(f5);
            }
        }
        ...
        BaseLayer baseLayer = this.matteLayer;
        if (baseLayer != null) {
            baseLayer.setProgress(f5);
        }
        for (int i6 = 0; i6 < this.animations.size(); i6++) {
            this.animations.get(i6).setProgress(f5);
        }
    }
}
