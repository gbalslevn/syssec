...
public abstract class BaseKeyframeAnimation<K, A> {
    ...
    public static final class KeyframesWrapperImpl<T> implements KeyframesWrapper<T> {
        ...
        private Keyframe<T> findKeyframe(float f5) {
            ...
            for (int size = this.keyframes.size() - 2; size >= 1; size--) {
                ...
                if (this.currentKeyframe != keyframe2 && keyframe2.containsProgress(f5)) {
                    return keyframe2;
                }
            }
        ...
        public boolean isCachedValueEnabled(float f5) {
            ...
            if (keyframe == keyframe2 && this.cachedInterpolatedProgress == f5) {
                return true;
            }
            this.cachedCurrentKeyframe = keyframe2;
            this.cachedInterpolatedProgress = f5;
            return false;
        }
        ...
        @Override // com.airbnb.lottie.animation.keyframe.BaseKeyframeAnimation.KeyframesWrapper
        public boolean isValueChanged(float f5) {
            if (this.currentKeyframe.containsProgress(f5)) {
                return !this.currentKeyframe.isStatic();
            }
            this.currentKeyframe = findKeyframe(f5);
            return true;
        }
    }
    ...
    public static final class SingleKeyframeWrapper<T> implements KeyframesWrapper<T> {
        ...
        @Override // com.airbnb.lottie.animation.keyframe.BaseKeyframeAnimation.KeyframesWrapper
        public boolean isCachedValueEnabled(float f5) {
            if (this.cachedInterpolatedProgress == f5) {
                return true;
            }
            this.cachedInterpolatedProgress = f5;
            return false;
    ...
    abstract A getValue(Keyframe<K> keyframe, float f5);
    ...
    public void setProgress(float f5) {
        ...
        if (f5 < getStartDelayProgress()) {
            f5 = getStartDelayProgress();
        } else if (f5 > getEndProgress()) {
            f5 = getEndProgress();
        }
        if (f5 == this.progress) {
            return;
        ...
        this.progress = f5;
        if (this.keyframesWrapper.isValueChanged(f5)) {
            notifyListeners();
        }
    }
    ...
    protected A getValue(Keyframe<K> keyframe, float f5, float f6, float f7) {
