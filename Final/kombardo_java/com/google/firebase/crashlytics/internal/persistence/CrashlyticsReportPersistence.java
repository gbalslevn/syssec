...
public class CrashlyticsReportPersistence {
    ...
    private static final Comparator<? super File> LATEST_SESSION_ID_FIRST_COMPARATOR = new Comparator() { // from class: L0.a
    ...
    private static final FilenameFilter EVENT_FILE_FILTER = new FilenameFilter() { // from class: L0.b
    ...
    private static long convertTimestampFromSecondsToMs(long j5) {
        return j5 * 1000;
    }
    ...
    private static String generateEventFilename(int i5, boolean z5) {
        return EVENT_FILE_NAME_PREFIX + String.format(Locale.US, EVENT_COUNTER_FORMAT, Integer.valueOf(i5)) + (z5 ? PRIORITY_EVENT_SUFFIX : "");
    }
    ...
    private void synthesizeNativeReportFile(File file, CrashlyticsReport.FilesPayload filesPayload, String str, CrashlyticsReport.ApplicationExitInfo applicationExitInfo) {
        ...
        try {
            ...
            Logger.getLogger().w("Could not synthesize final native report file for " + file, e5);
        }
    }

    private void synthesizeReport(String str, long j5) {
        boolean z5;
        ...
        loop0: while (true) {
            ...
            for (File file : sessionFiles) {
                try {
                    ...
                    Logger.getLogger().w("Could not add event to report for " + file, e5);
                }
                if (z5 || isHighPriorityEventFile(file.getName())) {
                    z5 = true;
                }
            }
        }
        if (!arrayList.isEmpty()) {
            synthesizeReportFile(this.fileStore.getSessionFile(str, REPORT_FILE_NAME), arrayList, j5, z5, UserMetadata.readUserId(str, this.fileStore), this.sessionsSubscriber.getAppQualitySessionId(str));
        } else {
            Logger.getLogger().w("Could not parse event files for session " + str);
        }
    }

    private void synthesizeReportFile(File file, List<CrashlyticsReport.Session.Event> list, long j5, boolean z5, String str, String str2) {
        try {
            CrashlyticsReportJsonTransform crashlyticsReportJsonTransform = TRANSFORM;
            CrashlyticsReport withEvents = crashlyticsReportJsonTransform.reportFromJson(readTextFile(file)).withSessionEndFields(j5, z5, str).withAppQualitySessionId(str2).withEvents(list);
            CrashlyticsReport.Session session = withEvents.getSession();
            if (session == null) {
                return;
            }
            Logger.getLogger().d("appQualitySessionId: " + str2);
            writeTextFile(z5 ? this.fileStore.getPriorityReport(session.getIdentifier()) : this.fileStore.getReport(session.getIdentifier()), crashlyticsReportJsonTransform.reportToJson(withEvents));
        } catch (IOException e5) {
            Logger.getLogger().w("Could not synthesize final report file for " + file, e5);
        }
    }

    private int trimEvents(String str, int i5) {
        List<File> sessionFiles = this.fileStore.getSessionFiles(str, new FilenameFilter() { // from class: L0.c
            @Override // java.io.FilenameFilter
        ...
        Collections.sort(sessionFiles, new Comparator() { // from class: L0.d
            @Override // java.util.Comparator
            public final int compare(Object obj, Object obj2) {
    ...
    public List<CrashlyticsReportWithSessionId> loadFinalizedReports() {
        ...
        for (File file : allFinalizedReportFiles) {
            try {
                ...
                Logger.getLogger().w("Could not load report file " + file + "; deleting", e5);
                file.delete();
            }
        }
        return arrayList;
    ...
    public void persistEvent(CrashlyticsReport.Session.Event event, String str, boolean z5) {
        ...
        try {
            writeTextFile(this.fileStore.getSessionFile(str, generateEventFilename(this.eventCounter.getAndIncrement(), z5)), TRANSFORM.eventToJson(event));
        } catch (IOException e5) {
            Logger.getLogger().w("Could not persist event for session " + str, e5);
        }
        trimEvents(str, i5);
    }
    ...
    public void persistReport(CrashlyticsReport crashlyticsReport) {
        ...
        try {
            ...
            Logger.getLogger().d("Could not persist report for session " + identifier, e5);
        }
    }

    private static void writeTextFile(File file, String str, long j5) {
        OutputStreamWriter outputStreamWriter = new OutputStreamWriter(new FileOutputStream(file), UTF_8);
        try {
            outputStreamWriter.write(str);
            file.setLastModified(convertTimestampFromSecondsToMs(j5));
            outputStreamWriter.close();
        } catch (Throwable th) {
            try {
                outputStreamWriter.close();
