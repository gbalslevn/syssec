...
public abstract class LazyGridMeasuredItemProvider implements LazyLayoutMeasuredItemProvider<LazyGridMeasuredItem> {
    ...
    public abstract LazyGridMeasuredItem mo394createItemO3s9Psw(int index, Object key, Object contentType, int crossAxisSize, int mainAxisSpacing, List<? extends Placeable> placeables, long constraints, int lane, int span);

    /* renamed from: getAndMeasure-m8Kt_7k, reason: not valid java name */
    public final LazyGridMeasuredItem m398getAndMeasurem8Kt_7k(int index, long constraints, int lane, int span, int mainAxisSpacing) {
        ...
        List<Placeable> mo422measure0kLqBqw = this.measureScope.mo422measure0kLqBqw(index, constraints);
        if (Constraints.m2576getHasFixedWidthimpl(constraints)) {
            m2579getMinHeightimpl = Constraints.m2580getMinWidthimpl(constraints);
        } else {
            if (!Constraints.m2575getHasFixedHeightimpl(constraints)) {
                throw new IllegalArgumentException("does not have fixed height");
            }
            m2579getMinHeightimpl = Constraints.m2579getMinHeightimpl(constraints);
        }
        return mo394createItemO3s9Psw(index, key, contentType, m2579getMinHeightimpl, mainAxisSpacing, mo422measure0kLqBqw, constraints, lane, span);
    }
    ...
    public LazyGridMeasuredItem mo375getAndMeasurehBUhpc(int index, int lane, int span, long constraints) {
        return m398getAndMeasurem8Kt_7k(index, constraints, lane, span, this.defaultMainAxisSpacing);
    }
}
